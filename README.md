...
#### 方法 B：使用預編譯映像檔 (最快)
修改 `docker-compose.yml` 中的 `bot` 服務，將 `build: .` 替換為：
...
```bash
# 將所有紀錄匯出為 backup.sql
sudo docker exec -t dpx-db-postgres pg_dumpall -c -U dpx_user > backup.sql
```
...
sudo tar -czvf dpx_bot_backup.tar.gz .
```
...
```bash
# 將備份的 SQL 倒回資料庫
cat backup.sql | sudo docker exec -i dpx-db-postgres psql -U dpx_user
```
