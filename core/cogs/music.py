...
class music_bot(Cog_Extension):
    ...
    def __init__(self, bot):
        super().__init__(bot)
        self.logger = logging.getLogger("dpx-discord-bot.music")
    ...
    def _get_best_thumbnail(self, thumbnails):
        """
        解析 thumbnails 列表，抓取解析度最佳的項目
        maxresdefault等預設項目經常不是有效的URL，因此改為檢查解析度指標，並回傳解析度最高的 URL。
        """
        ...
        for thumb in thumbnails:
            ...
            if w and h:
                ...
                # 嘗試解析 resolution 字串 "1280x720"
                res_str = thumb.get("resolution")
                if res_str and "x" in res_str:
                    ...
                    res = 0
            if res > max_res or best_url is None:
                max_res = res
                best_url = url
                
        return best_url

    def _fill_music_metadata(self, music, info_dict, fetch_thumb=False):
        ...
        # 僅在 fetch_thumb 為 True 時才抓取縮圖 (延遲載入)
        if fetch_thumb:
            music.thumbnail_url = self._get_best_thumbnail(info_dict.get("thumbnails", [])) or info_dict.get("thumbnail")
    ...
    async def getYTMusicInfo(self, url):
        ...
        with YoutubeDL(self.ytdl_opts) as ytdl:
            ...
            # 初次解析不抓取縮圖以節省空間
            self._fill_music_metadata(music, infoDict, fetch_thumb=False)
        return music
    ...
    async def renewYTMusicInfo(self, music):
        ...
        with YoutubeDL(self.ytdl_opts) as ytdl:
            ...
            self._fill_music_metadata(music, infoDict, fetch_thumb=True)
    ...
    def _create_music_detail_from_entry(self, entry, music_type):
        """從播放清單的 entry 快速建立 MusicDetail (不包含縮圖)"""
    ...
    async def audioPlayerTask(self, interaction: discord.Interaction, sendInteractionFollowupForOnce=False):
        ...
        while True:
            ...
            try:
                ...
                if current_music.type == self.TYPE_YOUTUBE:
                    try:
                ...
                elif current_music.type == self.TYPE_GDRIVE:
                ...
                elif current_music.type == self.TYPE_BILIBILI:
                    try:
    ...
    async def _internal_add(self, interaction: discord.Interaction, url: str):
        ...
        # URL 標準化
        url = url.replace("www.youtube.com/", "youtube.com/").replace("music.youtube.com/", "youtube.com/").replace("m.youtube.com/", "youtube.com/").replace("youtu.be/", "youtube.com/watch?v=").replace("youtube.com/shorts/", "youtube.com/watch?v=")
        ...
        if "www.youtube.com" in url:
            if "list=" in url:
                try:
                    ...
                    if not entries: failed_items.append(url)
                    else:
                        start_index = 0
                        if "watch?v=" in url:
                        ...
                        for entry in entries[start_index:]:
                            ...
                            except: failed_items.append(entry.get("title") or "解析失敗項目")
                except: failed_items.append(url)
            else:
                try:
            ...
            try:
                ...
                if m and not m.title.startswith("未經驗證的格式") and not m.title.startswith("檔案過大"):
                    self.redis.push_queue(guild_id, m.to_dict())
                    added_titles.append(m.title)
                else: failed_items.append(m.title if m else url)
            except: failed_items.append(url)
    ...
    async def play(self, interaction: discord.Interaction, url: str):
        ...
        if not await self._check_user_state(interaction):
            return
